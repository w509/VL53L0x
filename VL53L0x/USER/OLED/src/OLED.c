...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
	GPIO_InitTypeDef GPIO_InitStruct_OLED = { 0 };   //创建结构体变量  为什么要加一个初始化为零？？

	__HAL_RCC_GPIOB_CLK_ENABLE();   //使能 STM32 微控制器中的 GPIOB 外设的时钟

	GPIO_InitStruct_OLED.Pin = GPIO_PIN_8 | GPIO_PIN_9;  //用或 运算符 表示两个变量 可视为一个值 进行操作 ，即对两个引脚 同时设置
	GPIO_InitStruct_OLED.Mode = GPIO_MODE_OUTPUT_OD;
	GPIO_InitStruct_OLED.Pull = GPIO_NOPULL;     //把 SCL和SDA 设置为 不上拉也不下拉
	GPIO_InitStruct_OLED.Speed = GPIO_SPEED_FREQ_HIGH;       //frequent high???   非常高？？
	HAL_GPIO_Init(GPIOB, &GPIO_InitStruct_OLED);   //把配置好的 引脚初始化信息 以结构体 的方式 传递给 专门的GPIO引脚初始化函数 进行处理
...
void OLED_I2C_Start(void)
{
	...
	OLED_W_SCL(0);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接  ？？？ what does this mean？？
	// 可以说是 为紧接着的 SDA的 变化 做准备码？？
}
...
{
	OLED_W_SDA(0);		//拉低SDA，确保SDA为低电平   因为之后需要产生一个 上升沿来 代表 结束标志
	//但是万一这个时候 SCL 是 高电平的状态  那么不就会产生一个开始标志了码？？  难道这个不会影响数据传输码？？
	OLED_W_SCL(1);		//释放SCL，使SCL呈现高电平   SCL高电平期间 SDA 才能 跃变
...
void OLED_I2C_SendByte(uint8_t Byte)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		OLED_W_SCL(1);	//释放SCL，从机在SCL高电平期间读取SDA    这时候 从机 是如何读取数据的 ？？？
		OLED_W_SCL(0);	//拉低SCL，主机开始发送下一位数据
	}

	OLED_W_SCL(1);		//额外的一个时钟，不处理应答信号  与标准的 IIC协议并不完全相同， 可能OLDE从机并不需要发送ACK来声明数据被接收
	OLED_W_SCL(0);      //不直接拉低SCL 而是通过产生一个时钟脉冲 来提高系统的可靠性和稳定性
}
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	OLED_I2C_SendByte(0x78);		//发送OLED的I2C从机地址   这时候  SDA总线上面的 所有 slave 都会  把自己的地址和这个数据进行对比
	// 如果相同 就 输出 漏极低电平  否则 依旧保持 高阻态 即开路状态
	OLED_I2C_SendByte(0x00);		//控制字节，给0x00，表示即将写命令
	OLED_I2C_SendByte(Command);		//写入指定的命令    这个 command 和 data' 有啥子 不同呀？？
	OLED_I2C_Stop();				//I2C终止
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	OLED_I2C_SendByte(0x40);		//控制字节，给0x40，表示即将写数量 即将写数据 吧？？
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i++)
	{
